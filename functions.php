...
//	ADDS CUSTOM FIELDS for dimensions and media of artwork.
//  Taken from http://sltaylor.co.uk/blog/control-your-own-wordpress-custom-fields/
//  Steve Lambert added this on 4/22/10 and he's not sure it works.

if ( !class_exists('WPFolioCustomFields') ) {  

	class WPFolioCustomFields {  
			/** 
			...
			*/
			var $customFields =	array(
				array(
					"name"			=> "media",
					"title"			=> "Media",
					"description"	=> "media of the work (i.e. photography, graphite on paper, etc.)",
					"type"			=> "textarea",
					"scope"			=>	array( "page" ),
				...
				array(
					"name"			=> "collaborators",				
					"title"			=> "Collaborators",
					"description"	=> "if solo work, leave blank",
					"type"			=> "textarea",
					"scope"			=>	array( "page" ),
					"capability"	=> "edit_pages"
					),
		...
		*/
		function WPFolioCustomFields() { $this->__construct(); }
		...
		*/
		function createCustomFields() {
			if ( function_exists( 'add_meta_box' ) ) {
				//we don't want custom fields for PAGES, so we're leaving this out.
				// add_meta_box( 'wpfolio-custom-fields', 'WPFolio Custom Fields', array( &$this, 'displayCustomFields' ), 'page', 'normal', 'high' );
				add_meta_box( 'wpfolio-custom-fields', 'WPFolio Custom Fields', array( &$this, 'displayCustomFields' ), 'post', 'normal', 'high' );
			}
		}
		...
		*/
		function displayCustomFields() {
			...
			<div class="form-wrap">
				<?php
				wp_nonce_field( 'wpfolio-custom-fields', 'wpfolio-custom-fields_wpnonce', false, true );
			...
			<?php
				...
				*/
				function saveCustomFields( $post_id, $post ) {
					if ( !wp_verify_nonce( $_POST[ 'wpfolio-custom-fields_wpnonce' ], 'wpfolio-custom-fields' ) )
						return;
		...
		if ( class_exists('WPFolioCustomFields') ) {
			$WPFolioCustomFields_var = new WPFolioCustomFields();
		}				
//END CUSTOM FIELDS code. ...if it even works.		
...
<div class="wrap"> <!-- WHERE DOES THIS DIV GET CLOSED? -->
